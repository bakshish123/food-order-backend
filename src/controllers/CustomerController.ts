...
// |---> Send OTP via SMS
// |---> Generate JWT Token
// |---> Respond with signature & verification status
// |
// |
// User Verifies OTP (PATCH /verify)
// |
...
//updating the jwt signature everytime we change the verified status as when we will be validating the signature 
// as the payload has changed so will the signature hence validation will fail hence generate new sign everytime 
// payload is alterd
...
const assignOrderForDelivery = async(orderId: string, vendorId: string) => {
    ...
    if(vendor){
        ...
        if(deliveryPerson){
            ...
            //right now just assigning the first one in the deliveryPerson arr
        ...
        foods.map(food => {
            cart.map(({ _id, unit}) => {
                if(food._id == _id){
                    ...
                    cartItems.push({ foodId: food._id, unit });
                }
            })
        })
...
// for AddToCart
// 1. Receive Request (foodId, unit)  
// 2. Authenticate User ✅  
//    ├── User Exists → Fetch User's Cart  
//    ├── User Not Found → Return 404  
// 3. Fetch Food from Database ✅  
//    ├── Food Exists → Continue  
//    ├── Food Not Found → Return 404  
// 4. Process Cart ✅  
//    ├── If Cart is Empty → Add New Item  
//    ├── If Item Exists in Cart  
//    │    ├── If unit > 0 → Update Quantity  
//    │    ├── If unit = 0 → Remove Item  
